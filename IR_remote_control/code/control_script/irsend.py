...
Usage:
  ...
  connect       open connection with device and initialise transmission duration
  ...
  set           modify the configuration file
    profile     set the profile to use
    device      set the device to use
    txmax       set the default maximum transmission duration multiplier [1-9]

  txmax         change the current maximum transmission duration multiplier [1-9]
...
class Arduino():
    ...
    def setTxmax(self, value):
        if self.connected:
            print('Set Tx max multiplier to \"%s\".' % value)
            s = 'TMAX ' + value + '\r\n'
            try:
                self.ser.write(s.encode())
                s = self.ser.readline().decode()
...
def set_config(key, value):
    if key=='profile' or key=='device' or key=='txmax' :
        with open('irsend_config.json','r+') as file:
            config = json.load(file)
            config[key] = value
            file.seek(0)
...
if __name__ == "__main__":
    ...
    # interactive menu
    if(len(sys.argv) < 2):
        histfile = os.path.join(os.path.expanduser("~"), ".irsend_history")
        try:
        ...
        try:
            ...
            while done is False:
                ...
                if cmd[0] == 'exit' or cmd[0] == 'quit' or cmd[0] == 'q':
                    ...
                    arduino.connect(config['device'], baud_rate=baudrate, timeout=5)
                    arduino.setTxmax(config['txmax'])
                elif cmd[0] == 'show':
                    if len(cmd) < 2:
                    ...
                    if len(cmd) < 2:
                        print_error('missing argument')
                    elif cmd[1] == 'profile' or cmd[1] == 'device' or cmd[1] == 'txmax':
                        if len(cmd) < 3:
                            print_error('missing argument')
                        else:
                            arduino.disconnect()
                    ...
                    if len(cmd) < 2:
                        ...
                        if (int(cmd[1]) >= 1) and (int(cmd[1]) <= 9):   
                            arduino.setTxmax(cmd[1])
                        else:
                            print_error('Tx max must be in the range [1-9]')            
                elif cmd[0] == 'send':
                    if len(cmd) < 2:
                        print_error('missing argument')
                    else:
    ...
    # command line interface
        ...
        if command == 'send':
            if value in encoding[profile].keys():
                arduino.connect(device, baud_rate=baudrate, timeout=5)
                arduino.setTxmax(txmax)
