...
#define TOTAL_CYCLES 10000  // reduced value to avoid repeated entries
...
ISR(TIMER1_OVF_vect) {
  ...
  if (ir_cycle_count== ir_code[ir_seq_index]) {
    ...
    /*  alternate algorithm : send a single frame
    if ((ir_seq_index >= sequenceIndexEnd ) || (ir_total_cycle_count>TOTAL_CYCLES)) {
        ir_off();
        TCCR1B &= ~(1<<CS10);
    }
    */
